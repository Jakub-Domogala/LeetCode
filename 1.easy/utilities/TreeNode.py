...
class TreeNode(object):
    ...
    def get_max_depth(self) -> int:
        return max(self.left.get_max_depth() if self.left else 0, self.right.get_max_depth() if self.right else 0) + 1
    
    def get_min_depth(self) -> int:
        return min(self.left.get_min_depth() if self.left else 0, self.right.get_min_depth() if self.right else 0) + 1
    
    def get_count(self) -> int:
        return (self.left.get_count() if self.left else 0) + (self.right.get_count() if self.right else 0) + 1
    
    def get_min_val(self) -> int:
        return min(self.left.get_min_val() if self.left else inf, self.right.get_min_val() if self.right else inf, self.val)
    
    def get_max_val(self) -> int:
        return max(self.left.get_max_val() if self.left else -inf, self.right.get_max_val() if self.right else -inf, self.val)
